...
本章介绍声学领域的两个关键波导，涵盖了声波传播的基本原理、质量守恒和动量守恒方程，以及声速剖面的非线性特性，讨论了不同声传播模型，包括简正模态、射线模型，以及它们的应用。 Ideal 波导提供了理想化声场环境的描述， Pekeris 波导考虑了更实际的两层介质模型， Kraken 简正波模型和 Bellhop 射线模型则分别用于不同频率范围的声场计算。最后推导了声速剖面以及如何使用 MATLAB 进行声场仿真。
