...
对于主动声纳，作用距离可以通过以下方程计算：

$$
\text{DT} = \text{SL} - 2 \times \text{TL} + \text{TS} - \text{NL} + \text{DI}
$$

其中，\(\text{SL}\) 是声源级，\(\text{TL}\) 是传输损失，\(\text{TS}\) 是目标强度，\(\text{NL}\) 是噪声级，\(\text{DI}\) 是方向指数。
...
$$
\text{DT} = \text{SL} - \text{TL} + \text{DI} - \text{NL}
$$

#### 通信声纳的作用距离

通信声纳的作用距离计算公式与被动声纳类似，不过需要加上通信损失（\(\text{CL}\)）：

$$
\text{DT} = \text{SL} - \text{TL} - \text{CL} + \text{DI} - \text{NL}
$$

#### 优质因数 (FOM) 与作用距离的关系

优质因数（\(\text{FOM}\)）是声纳系统性能的一个指标，它结合了传输损失和其他几个参数。计算公式为：

$$
\text{FOM} = \text{SL} - \text{NL} + \text{DI} - \text{TL}
$$

通过这个公式，可以评估声纳系统探测某个目标的能力。理论上，\(\text{FOM}\) 越高，声纳系统的作用距离越远。
