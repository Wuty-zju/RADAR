...
本章节深入探讨声学领域中重要的 Ideal 波导和 Pekeris 波导，涉及声波传播的基本原理，包括质量守恒、动量守恒方程及声速剖面的非线性特性，详细讨论了简正模态和射线模型及其在声学中的应用。 Ideal 波导为理想化声场提供描述， Pekeris 波导则考虑实际的两层介质模型， Kraken 模型和 Bellhop 模型分别针对不同频率范围的声场进行计算，最后讨论声速剖面的推导及使用 MATLAB 进行声场仿真。
...
本章节详细探讨了波形生成、模糊度函数的计算与分析、以及水声信道响应的仿真模拟，涵盖了窄带信号、基带复包络、希尔伯特变换、正交解调和解析信号的基本概念，重点介绍了多普勒频移、匹配滤波器、脉冲压缩、模糊函数等核心内容。通过 MATLAB 仿真模拟，展示了单脉冲信号、高斯脉冲信号、线性调频信号的模糊函数计算，包括它们的三维表面图、3dB衰减等高线图以及零多普勒频移和零时间延迟时的模糊函数图。同时结合水声信道的特性，进行了单位脉冲响应、归一化脉冲响应、距离时延以及信噪比和误码率的仿真分析，深入探讨了水声信道对信号传输的影响，特别是在不同条件下信号的传播和接收质量。
...
当 $TB = 1$ 时，信号为最佳匹配脉冲，提供最好的时间解析能力。当 $TB \gg 1$ 时，信号可以提供更好的多普勒解析能力。
...
复杂信号在雷达和声纳系统中指的是时间带宽积 $TB \gg 1$ 的信号。这类信号在时间域或频域具有高分辨率，使得系统能够精确测定目标的距离和速度。
...
本章节详细介绍了声纳与雷达检测中常用的几种检测方法，恒虚警率检测 CFAR 、最大似然检测 ML 以及贝叶斯检测，并阐述了它们之间的联系。 CFAR检测作为一种在给定虚警率约束条件下优化检测概率的方法，其本质上是贝叶斯检测在特定条件下的应用形式，而 ML 检测则是在先验概率相等时的特例。同时章节中还讲解了贝叶斯公式在声纳与雷达检测中的应用，即如何利用后验概率、似然度和先验概率之间的关系来提高检测的准确性。此外还介绍了声纳检测阈的计算方法，包括被动声纳能量检测器和主动声纳匹配滤波器的检测阈计算，这些都基于虚警概率、检测概率和时间带宽积的特定值。最后通过接收机工作特性曲线 ROC的分析，进一步理解了不同检测方法在操作特性上的差异及其应用场景。
...
在声纳与雷达参数的估计中，最大似然估计 MLE 和最大后验估计 MAP 分别是基于概率模型的参数估计方法， MLE 通过最大化似然函数来估计参数，而 MAP 在此基础上考虑了参数的先验分布，这两种方法在特定条件下可以相互转换，对于声纳和雷达系统中的距离、速度和方位的估计问题，克拉美-罗界 CRB 提供了一个理论下限，指出无偏估计量的方差不可能低于这一界限。在阵列波束形成技术中，均匀直线阵的常规波束形成 CBF 、最小无失真响应 MVDR 、多信号分类 MUSIC 和旋转不变子空间 ESPRIT 方法各具特色， CBF 通过加权相加增强特定方向信号， MVDR 在保持对期望信号方向增益的同时抑制其他方向噪声， MUSIC 利用信号子空间和噪声子空间的正交特性估计信号方向， ESPRIT 则利用传感器阵列内部的几何关系进行估计。阵列协方差矩阵作为波束形成技术的核心，不仅捕捉接收信号的空间特性，还用于自适应调整波束形成权重，协方差矩阵的特征值分解、奇异值分解和 LU 分解等矩阵分解方法在波束形成中发挥着重要作用。通过MATLAB仿真，对这些波束形成技术进行深入的分析和比较。
...
声纳系统的作用距离计算是一个关键问题，涉及主动声纳、被动声纳和通信声纳三种类型，每种类型都有其特定的声纳方程来描述其性能，如主动声纳依据单基地混响控制、单基地噪声控制、多基地混响控制和多基地噪声控制的不同情景采用不同的方程，被动声纳则考虑声源级、噪声级、指向性指数和检测阈之间的关系，通信声纳则在形式上与被动声纳相似但在检测阈的计算上与主动声纳相同。优质因数 $FOM$ 是衡量声纳系统性能的关键指标，它与声纳方程中的传播损失 $TL$ 紧密相关，表现为声纳系统在特定环境下的有效探测距离，这一距离可以通过特定的传播损失公式或传播损失图计算得出，其中包括球面波和柱面波的传播损失公式，这些公式考虑了几何扩展损失和吸收损失。此外声纳系统的检测阈 $DT$ 也是计算作用距离的一个重要因素，它与声纳检测器的类型（如宽带能量检测器、窄带能量检测器或匹配滤波器）相关，具体形式取决于检测指数、处理带宽和积分时间。
